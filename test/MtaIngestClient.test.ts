...
describe("MtaIngestClient Tests", () => {
    ...
    describe("deliver()", () => {
        ...
        it("Percent-encodes a comma inside a quoted local part so it can't be split into an extra recipient.", async () => {
            // smtp-server does only "permissive validation" on RCPT TO - a quoted local part containing a
            // literal comma (RFC 5321 allows it) is accepted unchanged, so `envelopeTo` can legitimately
            // contain one. Joining recipients with a bare "," would then be ambiguous to split back apart.
            mockFetch(202);
        ...
        it("Sends a null sender (a bounce) as an empty X-Envelope-From header over real HTTP.", async () => {
            ...
            try {
                ...
                expect(seen).toEqual([{ from: "", to: encodeURIComponent("alice@owned.lab"), length: dsn.length }]);
            } finally {
    ...
    describe("upstream timeout", () => {
        ...
        // show whether an AbortSignal.timeout() actually cuts off a real stalled request. TcpTableServer
        // serializes lookups per Postfix-held connection, so without a timeout here a single hung upstream
        // call would stall every queued lookup on that connection forever (and, for deliver(), stall the
        // one in-flight SMTP transaction).
        ...
        async function closeHangingServer(server: http.Server): Promise<void> {
            // The stalled request's own connection is still open server-side even after the client aborts
            // in most Node versions by the time this runs, so force it closed rather than waiting on the
            // graceful net.Server.close() this class's own SmtpDeliveryServer/TcpTableServer are being
            // fixed to also do (see fix #4) - closeAllConnections() is Node's own equivalent primitive.
