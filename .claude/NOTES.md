...
#### Follow-up (same day) - the two new env vars had no input validation, reproducing the exact bug they exist to fix

A second-round review of the commit above (reproduced live against the real `smtp-server` package, not just read from source) found that neither new numeric env var was validated before use:

- **`MTA_BRIDGE_MAX_MESSAGE_SIZE` silently disabled the size cap entirely for `NaN`/`0`/`""`.** `smtp-server`'s `startDataMode()` computes `this._maxBytes = (maxBytes && Number(maxBytes)) || Infinity` - all three of those values are falsy, so `_maxBytes` becomes `Infinity` and `sizeExceeded` can never flip true again. Confirmed live: an oversized message sails through with a normal `250 OK` and no error/log/symptom of any kind - this is the exact unbounded-buffering vulnerability the whole size-cap fix above exists to close, reintroduced by an unvalidated config value. Not exploitable today since this env var isn't wired into Helm/docker-compose yet, but this file's own history (the DKIM/domain-list entries above) has more than one prior bug of exactly this "blank/placeholder value slips through templating" shape.
- **`MTA_INGEST_TIMEOUT_MS` fails the opposite (loud) way:** `AbortSignal.timeout(NaN)`/`(-1)` throws synchronously, which the `async` callers turn into a permanently-rejected promise - every domain check/recipient resolution/delivery would temp-fail from the first request onward with a generic error, not a targeted one.
- **Fix:** `requirePositiveNumber(name, fallback)` in `index.ts` - returns `fallback` when the env var is unset, otherwise requires `Number.isFinite(value) && value > 0` or throws `"<name> must be a positive number if set (got '<raw>')"` at startup, matching the existing `requireEnv()` fail-fast pattern already used for the string envs. Deliberately scoped to only the two numeric env vars this repo's own commit introduced (`MTA_BRIDGE_DOMAIN_PORT`/`RECIPIENT_PORT`/`SMTP_PORT` were pre-existing and out of this review's scope, left as bare `Number(...)` for now).
- **Tests:** `test/index.test.ts` adds a `describe.each` over both env vars x four bad values (`"not-a-number"`, `""`, `"0"`, `"-1"`) asserting `importIndexFresh()` rejects with the `"<name> must be a positive number"` message - covers both branches of `!Number.isFinite(value) || value <= 0` (the pre-existing valid-override test already covered the "both false" pass-through branch, and the pre-existing default-value test already covered the unset/`raw === undefined` branch). 100% coverage maintained (150/150 statements, 59/59 branches).
- Lint note: the JSDoc bullet-list format on `requirePositiveNumber`'s own doc comment tripped `jsdoc/check-indentation` the same way `MtaIngestClient.ts`'s `encodeEnvelopeAddress()` doc comment did in the commit above - this repo's ESLint config doesn't tolerate indented bullet continuation lines in JSDoc; wrote it as plain paragraphs instead, same fix as before.

#### Follow-up (2026-09-23) - Round 3: TcpTableServer's own line buffer was still unbounded, and MTA_INGEST_TIMEOUT_MS had no upper bound

A third-round review found two more gaps, both in the config this bridge itself introduced across the two commits above:

- **`TcpTableServer.handleConnection()`'s `buffer += chunk` had no size cap - the exact class of bug already fixed on the SMTP side, never mirrored here.** Worse blast radius than the SMTP-side version: this handler is synchronous and outside any `try`/`catch`, so a client on the `tcp_table` port that never sends `"\n"` (or sends one very long line) either exhausts this *process's* memory - taking down both `tcp_table` listeners and the SMTP listener with it, not just one connection - or, if V8's string-length ceiling is hit first, throws an uncaught `RangeError` that crashes the whole process. Fixed by tracking `buffer.length` against a new `maxLineLength` constructor param (`DEFAULT_MAX_LINE_LENGTH = 8192`, configurable via `MTA_BRIDGE_MAX_LINE_LENGTH`, validated with the same `requirePositiveNumber()` as the other two numeric env vars) and calling `socket.destroy()` once it's exceeded - checked *after* draining every complete `"\n"`-terminated line already in the buffer (not before), so a burst of legitimate pipelined requests isn't punished just because the next line hasn't finished arriving yet. No response is written before destroying: once a line has grown this large the framing can't be trusted, so this drops the connection outright rather than risk writing into the middle of whatever the client is still sending.
  - Added `maxLineLength` as the *third* constructor param (after `closeTimeoutMs`), not the second, specifically to avoid breaking the existing close-timeout test's positional `new TcpTableServer(lookup, 20)` call - a reminder to check existing positional-arg call sites before inserting a new parameter into the middle of a constructor.
  - One test drives the "never sends `\n` at all" case (buffer grows past the cap with zero newlines, `lookup` never called); a second drives the "complete line then an oversized unterminated one in the same chunk" case, which exercises the "checked after the while-loop" code path specifically - and documents an accepted trade-off found while writing it: `socket.destroy()` runs synchronously in the same `"data"` callback, before the microtask queue ever reaches the already-scheduled `handleLine()` for the earlier, valid line, so that line's response never arrives either (its own `if (socket.destroyed) return` guard skips it). Postfix just sees the connection close before a response - the same "safe to retry" outcome as any other dropped connection - so this wasn't treated as its own bug.
- **`MTA_INGEST_TIMEOUT_MS` (made operator-configurable two commits ago) had no upper bound, but the two 30s close-timeout values it can collide with (`TcpTableServer`'s `DEFAULT_CLOSE_TIMEOUT_MS`, not wired to any env var, and `smtp-server`'s own hardcoded `CLOSE_TIMEOUT`, not configurable from this repo at all) are both still fixed.** If an operator sets it to something plausible-sounding for a slow upstream (45-60s) and a pod rolling-restart (SIGTERM) arrives while a `deliver()` call is in flight, the 30s force-close can destroy the socket before that call's response is sent. Postfix then sees a connection reset and retries the *whole transaction* - and unlike `reportUnresolvableRecipient()`, restapi's direct-mailbox delivery path has no idempotency key for this, so the retry becomes a duplicate delivery. Fixed by giving `requirePositiveNumber()` an optional `max` parameter and deriving `MAX_INGEST_TIMEOUT_MS = DEFAULT_CLOSE_TIMEOUT_MS - 5_000` (25s, imported from `TcpTableServer.ts` rather than hardcoded, so it tracks that constant if it ever changes) - `MTA_INGEST_TIMEOUT_MS` now must be `< 25000` or startup fails fast with the bound named in the error message. Chose "cap the timeout" over "make the close-timeouts configurable too" per the review's own steer (simpler, and `smtp-server`'s own close timeout isn't configurable from this repo regardless, so capping the timeout is the only fix that actually closes the gap for both collisions at once).
- **Coverage:** 100% (157/157 statements, 66/66 branches). `test/index.test.ts`'s mocked `TcpTableServer` constructor now also captures its 2nd/3rd (`closeTimeoutMs`/`maxLineLength`) args so the wiring itself is asserted, not just the lookup callback.
- `RELEASE_NOTES.md`: appended two more bullets to the existing `## Unreleased`/`### postfix-bridge` section (still unreleased, so no new version heading).
